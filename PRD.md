...
VibeCheck.lol is a small personal desktop tool that answers one question over time: **"When do I actually have fun playing League of Legends?"**
...
Local capture stays exactly as-is. This adds a **zero-config** shared layer: with no signup and no invite codes, your *rated* games sync to a shared backend and your squad — **your League friends who also run VibeCheck** — see leaderboards and the mutual-fun matrix. A player whose backend isn't configured is unaffected and fully local.
...
| Grouping | **Mutual League friends.** Your squad = friends from the LCU friends list who also run VibeCheck and list you back. The mutual friend link replaces the invite code as the consent gate. No "create squad" step. |
